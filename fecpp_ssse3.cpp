 ...
 */
alignas(64) const uint8_t GFTBL[] = {
...
size_t addmul_ssse3(uint8_t z[], const uint8_t x[], uint8_t y, size_t size)
   ...
   while(size >= 16)
      ...
      const __m128i z_1 = _mm_load_si128((const __m128i*)(z));

      // mask to get LO nibble for LO LUT input
      const __m128i x_lo = _mm_and_si128(x_1, mask);
      // mask to get HI nibble for HI LUT input
      const __m128i x_hi = _mm_and_si128(_mm_srli_epi64(x_1, 4), mask);

      // 16x parallel lookups
      const __m128i r_lo = _mm_shuffle_epi8(t_lo, x_lo);
      const __m128i r_hi = _mm_shuffle_epi8(t_hi, x_hi);

      // sum the outputs.
      __m128i s = _mm_xor_si128(z_1, _mm_xor_si128(r_hi, r_lo));

      _mm_store_si128((__m128i*)(z), s);
